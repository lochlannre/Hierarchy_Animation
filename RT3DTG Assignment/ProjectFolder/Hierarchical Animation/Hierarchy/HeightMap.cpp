...
HeightMap::HeightMap(char* filename, float gridSize, CommonApp::Shader *pShader)
{
	...
	//Make Sure pointers are safe / NULL

	LoadHeightMap(filename, gridSize);

	m_pHeightMapBuffer = NULL;

	static VertexColour MAP_COLOUR(200, 255, 255, 255);
	...
	for(int l = 0; l < m_HeightMapLength; ++l)
	{
		for(int w = 0; w < m_HeightMapWidth; ++w)
		{
			if(w < m_HeightMapWidth - 1 && l < m_HeightMapLength - 1)
			{
				...
				//Spread Texture evenly across landscape
				t0 = (v0 + vOffset) / 32.0f;
				t1 = (v1 + vOffset) / 32.0f;
				t2 = (v2 + vOffset) / 32.0f;
				t3 = (v3 + vOffset) / 32.0f;
...
//Input index and Row, Return the smoothed normal of the given index
...
XMFLOAT3* HeightMap::GetFaceNormalPtr(int faceIndex, int offset)
{
	...
	if (faceIndex >= m_HeightMapFaceCount)
		return &vUp;
...
void HeightMap::Draw(float frameCount)
{
	...
	// The D3D11_MAP_WRITE_DISCARD flag is best of performance
	// However,  when mapping, the previous buffer contents are indeterminate. so the entire buffer must be written
	if (m_pMyAppCBuffer)
	{
	...
	Application::s_pApp->DrawWithShader(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST, m_pHeightMapBuffer, sizeof(Vertex_Pos3fColour4ubNormal3fTex2f), NULL, 0, m_HeightMapVtxCount, NULL, m_pSamplerState, &m_shader);
}
...
bool HeightMap::ReloadShader(void)
{
	...
	if (!CompileShadersFromFile(pDevice, "./DrawHeightMap.hlsl", "VSMain", &pVS, &vs, g_aVertexDesc_Pos3fColour4ubNormal3fTex2f, g_vertexDescSize_Pos3fColour4ubNormal3fTex2f, &pIL, "PSMain", &pPS, &ps, aMacros))
	{
		return false;
	}
	...
	//find cbuffer(s) for the globals that won't get set by the CommonApp code.
	//These are shader-specific: oyu have to know what you are looking for, if you're going to set it...

	//Find the constant buffer which might in either shader or both.
	//However, it shoud be the same size and parameter offsets in either.
	...
	//Create a cbuffer, using the shader description to find out how large it needs to be.
	if (cBufferSize > 0)
	{
